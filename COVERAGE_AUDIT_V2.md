# cryptokit-rs coverage audit v2 (vs MacOSX26.2.sdk)
...
| --- | --- | --- | --- | --- |
| `CryptoKitError` | enum | `CryptoKit.swiftinterface` | The crate exposes a Rust-native `error::CryptoKitError` status mapping instead of mirroring Swift's throw-only enum cases verbatim. | Swift-level error enum; semantic coverage via Rust error type |
| `CryptoKitASN1Error / CryptoKitMetaError / CorecryptoCurveType` | enum/typealias/struct family | `CryptoKit.swiftinterface` | Swift-only ASN.1/meta/corecrypto utility families are not independently actionable in the Rust API and remain intentionally unmapped. | Internal/diagnostic types not part of the public functional API surface |
