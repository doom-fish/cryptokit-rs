...
fn post_quantum_signatures_round_trip() -> Result<()> {
    ...
    assert!(mldsa65_public.verify_with_context(b"mldsa65-context", &contextual_signature, Some(b"ctx"))?);
    let restored =
        Mldsa65PrivateKey::from_seed_representation(mldsa65.seed_representation()?, Some(&mldsa65_public))?;
    assert_eq!(restored.public_key()?.raw_representation(), mldsa65_public.raw_representation());
    ...
    assert!(mldsa87_public.verify_with_context(b"mldsa87-context", &contextual_signature, Some(b"ctx"))?);
