...
fn main() -> Result<()> {
    let signing = SigningPrivateKey::generate_with_compact_representable(SigningAlgorithm::P256, true)?;
    ...
    assert_eq!(restored_signing.raw_representation(), signing.raw_representation());
    assert_eq!(restored_verifying.raw_representation(), verifying.raw_representation());

    let agreement =
        KeyAgreementPrivateKey::generate_with_compact_representable(KeyAgreementAlgorithm::P256, true)?;
    let agreement_public = agreement.public_key()?;
    let agreement_x963 = agreement_public.x963_representation()?;
    let restored_agreement_public =
        KeyAgreementPublicKey::from_x963_representation(KeyAgreementAlgorithm::P256, agreement_x963)?;
    assert_eq!(
    ...
    println!("agreement-key-bytes={}", agreement.raw_representation().len());
    Ok(())
}
